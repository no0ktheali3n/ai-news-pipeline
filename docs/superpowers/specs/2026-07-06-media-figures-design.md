...
- **URL resolver (claims-audited 2026-07-06):**
  1. `src` starts with `http` → use as-is only if host is `arxiv.org`, else drop the candidate;
  2. `src` carries a `<id>v<n>/` leading segment → resolve as `https://arxiv.org/html/` + `src` — **the only form observed live (118/118 figure srcs across 10 audited papers)**;
  3. otherwise (bare or other relative) → resolve as `<response.url rstrip '/'>` + `/` + `src` — **defensive branch**: bare-src and `extracted/` forms were reported by reviewers but NOT reproduced in the audit (the "bare src on 2511.04694" claim was falsified — that paper's srcs are version-prefixed). Branch kept as cheap insurance with a synthetic fixture.
  The resolved URL must then GET as `200` with `Content-Type: image/*` — the fetch below is the final arbiter for every branch; 404 → drop the candidate.
...
**Fixtures — CAPTURED from real pages, never hand-invented (TDD requirement):** a plan task snapshots the actual HTML of 2607.02116 (version-prefixed srcs, **real .jpg figures** — the "PNG-only lane" claim was falsified in the claims audit, so the JPEG dimension parser gets a real fixture — plus the table-cell `CC-BY-4.0` license decoys and a CC license anchor), 2607.01641 (non-CC "arXiv.org perpetual" anchor), and 2607.01600 (MathML caption) into `tests/fixtures/`; table-first `<figure>` and multi-`<img>` subfigure grids appear in these captures (asserted excluded). Synthetic fixtures only where no real instance exists: bare-src / other-relative resolver branch, PNG/JPEG header bytes for the dimension parser's edge cases.
...
## Claims audit (2026-07-06, owner-mandated — every "verified" statement re-checked first-hand)

Live audit: 10 lane papers, plus grep-verification of all 9 code-level claims. **VERIFIED:** no-redirect behavior (0/10 versioned final URLs); license anchor 10/10 parseable from text (distribution: 8/10 CC BY, 2/10 arXiv-perpetual → **the license gate costs ~20% of papers**); table-cell decoys real; multi-img subfigures real (21 found); dimension-gate recalibration direction (old gate 2/5 vs new 4/5 on first qualifying figures; note 2607.02514's Figure 1 at aspect 3.02 sits just outside the 3.0 bound — calibration-task datum); all code claims (4 strict-signature test fakes at the cited lines, stubs lacking tweepy.API/byte-support, 600/900 timeout pair, no pipeline-level retry, bs4 absent, tweepy 4.15-pin/4.17-env); media-upload entitlement (first-hand probe). **FALSIFIED and corrected:** "PNG-only lane / JPEG has no real fixture" (jpg figures exist on 2607.02116); "bare src on 2511.04694" (118/118 audited srcs are version-prefixed — bare/`extracted/` handling kept only as a defensive branch with synthetic fixtures). Fixture policy hardened: captured real pages, never hand-invented HTML.

**TDD (owner-mandated):** the implementation plan follows strict test-first discipline — every behavior lands as a failing test against captured-real fixtures before its implementation; no test may be written after the code it covers.
